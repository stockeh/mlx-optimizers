...
from mlx.optimizers import Adam, SGD, AdamW
...
def objective_rosenbrock(params):
    ...
    steps = execute_steps(rosenbrock, ROSENBROCK_INITIAL,
                          params['optimizer'](**optimizer_config), params['steps'])
    return ((steps[-1, 0] - ROSENBROCK_MINIMUM[0]) ** 2
            + (steps[-1, 1] - ROSENBROCK_MINIMUM[1]) ** 2).item()
...
def plot_rosenbrock(steps, optimizer_name, lr):
    ...
    ax.plot(ROSENBROCK_MINIMUM[0], ROSENBROCK_MINIMUM[1],
            c='k', marker='o', ms=6)
    ...
    fig.tight_layout()
    fig.savefig(
        f'../docs/media/rosenbrock_{optimizer_name}.png',
    ...
    for optimizer, lr_low, lr_hi, optimizer_kwargs in optimizers:
        ...
        if best:
            ...
            optimizer_instance = optimizer(
                learning_rate=best['learning_rate'], **optimizer_kwargs
            )
            steps = execute_steps(func, initial_state,
                                  optimizer_instance, space['steps'])
...
if __name__ == '__main__':
    ...
    execute_experiments(optimizers, objective_rosenbrock,
                        rosenbrock, plot_rosenbrock, ROSENBROCK_INITIAL)
